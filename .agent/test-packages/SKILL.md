...
### Models & DTOs
* **Serialization & Deserialization (`fromFireStore` / `fromJson`, `toFireStore` / `toJson`)**:
  * Test complete valid data conversion in both directions.
  * **Nullability & Optional Fields**: Verify that missing or `null` values for nullable properties (e.g. `String? image`) remain `null` and are NOT converted to non-null defaults (such as `''`).
  * **Null Field Serialization**: Verify that unset nullable properties serialize correctly as `null` in output maps.
  * **Enum Fallbacks**: Test fallback defaults when encountering unknown or missing string values for enums.
  * **Equatable Value Equality**: Verify that instances with identical values are equal (`props`).

### Mappers
* **File Separation Rule (CRITICAL)**: If mappers are defined in separate extension files (e.g. `my_user_mapper.dart`, `my_user_dto_mapper.dart`), **do NOT include mapper tests inside the Model or DTO test file**. Mapper tests belong exclusively in dedicated test files mirroring the mapper file location (e.g. `test/.../mappers/my_user_mapper_test.dart`).
* Test both `toEntity()` and `toDto()` (or `toUser()` and `toMyUserDto()`).
